...
# Streamlit Cloud'daki React removeChild hatasını önlemek için JS GPS komponenti devre dışı.
# Konum manuel/CBS veya mobil uygulama fazında alınır.
...
def supabase_analizleri_sil():
    """Jüri denemesi öncesi ortak arşivi temizler. Service role key gerektirir."""
    _, ayar, msg = supabase_client_getir()
    if not ayar.get("aktif"):
        return False, msg
    try:
...
def yardimci_sahne_tahmini(image_pil, mevcut_preds=None):
    ...
    kullanılır. Jüri demosunda yanlış iddia üretmemek için güven skorları düşük/orta tutulur.
    """
...
def cihaz_canli_konumunu_al(aktif_istek=False):
    ...
    if get_geolocation is None:
        return None, "Stabil demo modunda tarayıcı GPS komponenti kapalı. Lütfen manuel koordinat girin veya FlutterFlow mobil fazında cihaz GPS kullanın."
    try:
        loc = get_geolocation()
        if not loc:
            ...
            with st.expander(f"#{idx} {analiz['dosya']} | Hash: {analiz['hash_kisa']} | Durum: {analiz.get('risk_seviyesi','')}", expanded=(idx == len(aktif_analizler))):
                ...
                with c_res:
                    ...
                    if "EXIF GPS yok" in str(analiz.get("konum_kaynagi", "")) or analiz.get("cekim_tarihi") == "EXIF tarih yok":
                        st.warning("Bu görselde EXIF GPS/tarih bilgisi yok. Jüri demosunda yanlış veri üretmemek için çekim tarihi, saati ve koordinatı manuel doğrulayın veya anlık cihaz konumunu kullanın.")
...
with b7:
    temizle_onay = st.checkbox("Supabase sil", value=False, help="Jüri deneme kayıtlarını temizlemek için onay kutusunu işaretleyin.")
    if st.button("🧹 SB Temizle", use_container_width=True):
        if not temizle_onay:
